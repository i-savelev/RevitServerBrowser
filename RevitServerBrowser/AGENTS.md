...
- не запускай сборку плагина самостоятельно, пользователь сделает это стандартными инстрементами visual studio. Сообщай о готовности и о возможности собрать решение
...
- по умолчанию не показывать иконку окна через `ShowIcon = false`, если для формы не подготовлена отдельная полезная иконка
...
### Unicode-символы в форме

Для простых WinForms допустимо использовать Unicode-символы в `Text` кнопок, узлов дерева и меток, если это:

- улучшает читаемость интерфейса
- не требует отдельного ресурсного изображения
- стабильно отображается стандартным шрифтом Windows, обычно `Segoe UI`

Практические правила:

- предпочитать простые Unicode-символы, а не emoji, если нужен предсказуемый вид на разных машинах
- если символ на кнопке отображается как прямоугольник или пустой квадрат, заменить его на более простой Unicode-вариант или убрать вовсе
- для критичных элементов интерфейса, где важен одинаковый внешний вид, предпочитать `Image`/ресурс вместо символа в тексте
- не злоупотреблять декоративными символами, они должны помогать чтению, а не мешать

Примеры допустимого использования:

- `📁 Root` и `📁 {item.Name}` для папок в `TreeView`
- `[ ] {item.Name}` для невыбранной модели
- `[✓] {item.Name}` для выбранной модели
- `✅ Подтвердить` для основной кнопки подтверждения
- `✖ Закрыть` для кнопки закрытия

Если есть сомнение в отображении символа, по умолчанию выбирать более простой текстовый вариант или обычный текст без иконки.

### Строка состояния

Для интерактивных форм рекомендуется добавлять простую строку состояния, которая показывает пользователю, что происходит в текущий момент.

Строка состояния особенно полезна, если форма:

- загружает данные по сети
- строит дерево или список по шагам
- обновляет выбранные элементы
- выполняет действие, которое занимает заметное время

Практические правила:

- строка состояния должна быть всегда видна внизу формы
- текст должен быть коротким и конкретным
- статус должен обновляться при ключевых действиях пользователя и на важных этапах выполнения
- после завершения операции статус должен переходить в понятное итоговое состояние
- при ошибке статус должен явно сообщать, что операция не выполнена

Хорошие примеры статусов:

- `Готов`
- `Загрузка данных...`
- `Загружено: 24 элемента`
- `Выбрано: 3 элемента`
- `Выбор сброшен`
- `Ошибка загрузки`
...
private void FinishLoading(int count)
{
    _statusLabel.Text = $"Загружено: {count} элементов";
}

private void UpdateSelectionStatus(int selectedCount)
{
    _statusLabel.Text = $"Выбрано: {selectedCount} элементов";
}
